...
def MyDumbSAT_inc(wff, n_vars, n_clauses, assignment):

  # OPTIMIZATION: define a character set for each variable
  max_var = max(max(abs(i) for i in c) for c in wff)
  input_char_sets = []
  for i in range(max_var):
    input_char_sets.append([0, 1])

  # Use itertools.product to incrementally generate members of the Cross Product
